...
CANVAS_DEVELOPER_INSTRUCTIONS = (
    ...
    "the canvas, use the canvas dynamic tools as the primary interface and treat "
    "their current schemas as authoritative. Read the narrowest useful scene "
    "scope before scene-dependent edits, then apply bounded changes using the "
    "returned revision while preserving unrelated user content. Inspect the "
    "affected scope after material edits and address semantic lint errors before "
    "claiming completion. Treat backing and preview files as persistence "
    "artifacts rather than the canvas interface, and do not inspect or modify "
    "them unless the user explicitly requests it. Do not use external or offline "
    "canvas integrations unless the user explicitly requests one. Export only "
    "when the user asks for an export. If the canvas tools do not provide enough "
    "information, state the limitation instead of guessing or silently switching "
    "interfaces."
)
